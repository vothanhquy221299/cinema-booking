...
export default function MovieDetail(props) {
  //useParam() ~ props.match.params
  const {movieId} = useParams();

  //useDispatch ~ mapDispatchToProps
  const dispatch = useDispatch();
  
  //useSelector ~ mapStateT0Props
                      ...
                      <div className="text-left">
                        ...
                        <div className="my-4">
                          {cumRap.lichChieuPhim.map(lichChieu =>{
                            return (
                              <Link to={`/seat-plan/${lichChieu.maLichChieu}`} className="btn btn-secondary mr-3 mb-3">{new Date(lichChieu.ngayChieuGioChieu).toLocaleTimeString()}</Link>
                            )
                          })}
                        </div>
                      </div>
